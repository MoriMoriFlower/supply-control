 ...
 * ⑰出荷量のうち「普通に入ってくる」側。
 *
 * A．通常 と Aプラス．増加 は、現場から見れば同じ（発注すれば入る）。
 * ★この2つの行き来は通知しない。2026-08-17の実測では⑰の変化8件中5件が A⇄Aプラス で、
 *   通知に出しても何も起きないノイズだった。
 * ★ここに無い値は「普通ではない」側に倒す（＝通知する）。新しい区分が増えたとき、
 *   黙って見落とすより通知しすぎるほうが安全（医療情報の安全方針）。
 *   ただし空欄は「未報告」であって異常ではないので、判定側で普通扱いにしている。
 */
export const RYO_NORMAL = new Set(['A．出荷量通常', 'Aプラス．出荷量増加']);
